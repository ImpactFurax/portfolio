...
interface CardProps {
  items: { img: string; title: string, id: string }[];
}
...
const Card: React.FC<CardProps> = ({ items }) => {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 xl:gap-12">
      {items.map((item) => (
        <div key={item.id} style={{ backgroundImage: `url(/assets/images/${item.img})` }} className="w-full h-[300px] bg-cover bg-center bg-no-repeat rounded-xl flex flex-col items-center justify-center">
